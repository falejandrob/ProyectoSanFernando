...
@section('content')
    ...
    <div class="container d-flex justify-content-around align-items-center" style="border: black solid 1px; background: rgb(237, 237, 237); padding: 10px;">
        <div style="width: 50%">
            <form action="{{ route('establecerProveedor') }}" method="POST">
                ...
                @foreach ($lineasPedido as $linea)
                    ...
                    @foreach ($productosConProveedor as $item)
                        @if ($item->lineaPedido == $linea->id)
                            <div style="font-size: 150%; margin: 8px;">
                                ...
                                <span class="badge bg-dark">{{ App\Models\Proveedore::find($item->proveedor)->nombre }}</span>
                            </div>
                            @php($esta = true)
                        @endif
                    @endforeach
                    @if(!$esta)
                        <div style="font-size: 150%; margin: 8px;">
                            <input style="width: 20px; height: 20px; cursor: pointer;" type="checkbox" name="productos[]" value="{{ $linea->id }}">
                            <span>{{ App\Models\Producto::find($linea->idProducto)->nombre }}</span>
                        </div>
                    @endif
                @endforeach
        ...
        <div style="width: 50%;">
            @foreach ($proveedores as $proveedor)
                <div style="font-size: 150%;">
                    <input style="width: 20px; height: 20px; cursor: pointer;" type="radio" name="proveedor" value="{{ $proveedor->id }}">
                    {{ $proveedor->nombre }}
                </div>
            @endforeach
        </div>
