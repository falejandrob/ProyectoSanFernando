...
@section('content')
    <div class="container" style="padding: 25px">
        ...
        <h1 style="text-align: center;">INFORMES SEGÚN EL MES</h1>

        <br>
        <form action="{{ route("informesMesResultado") }}" method="post">
            @csrf
            @method("POST")
        ...
        @if($month != "")
            @if(count($labelsProductos) != 0)
                <h3 style="text-align: center;">Productos más pedidos del mes de {{ $monthNombre }}</h3>
                <div style="width: 60%; margin: auto;">
                    <canvas id="productos"></canvas>
                </div>
                <h3 style="text-align: center;">Categorias más pedidas del mes de {{ $monthNombre }}</h3>
                <div style="width: 60%; margin: auto;">
                    <canvas id="categorias"></canvas>
                </div>
            @else
                <h4 style="text-align: center;">En el mes de {{ $monthNombre }} no se ha hecho ningún pedido</h4>
            @endif
        @endif
    </div>
