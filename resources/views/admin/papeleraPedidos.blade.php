                ...
                <table class="table mb-0 tabla-scroll ">
                    ...
                    @foreach($pedidos as $pedido)
                        <tr class="" style="text-align: center;">
                            ...
                            <td id="informacion"
                                data-titulo="Profesor:">{{ $profesores->where("id","=",$pedido->idUser)->first()->nombre . " ". $profesores->where("id","=",$pedido->idUser)->first()->apellidos }}</td>
                            <td id="informacion"
                                data-titulo="Fecha pedido:">{{ explode(" ", $pedido->fechaPedido)[0] }}</td>
                            <td id="informacion"
                                data-titulo="Fecha prevista:">{{ explode(" ", $pedido->fechaPrevistaPedido)[0] }}</td>
                            <td id="informacion" data-titulo="Justificacion:">{{ $pedido->justificacion }}</td>
                            <td id="informacion" data-titulo="Restaurar:">
                                <a type="button" class="btn btn-success"
                                   href="{{ route('restaurarPedido', $pedido->id) }}">
                                    Restaurar
