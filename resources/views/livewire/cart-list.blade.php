        ...
        @foreach($cart as $productoCarrito)
            <div
                ...
                @if($productoCarrito->qty == 1)
                    <form wire:submit.prevent="removeFromCart('{{ json_encode($productoCarrito) }}')">
                    ...
                    <form wire:submit.prevent="addElementToProduct('{{ json_encode($productoCarrito) }}')">
                    <button type="submit" class="btn btn-danger">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                ...
                @elseif($productoCarrito->qty > 1)
                    <form wire:submit.prevent="restElementToProduct('{{ json_encode($productoCarrito) }}')">
                    <button type="submit" class="btn btn-danger">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                             class="bi bi-trash3" viewBox="3 3 26 26">
                            <path
